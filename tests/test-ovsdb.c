...
usage(void)
{
    printf("%s: Open vSwitch database test utility\n"
           ...
           "  idl-seqno-info SERVER [TRANSACTION...]\n"
           "  idl-delete-seqno SERVER [TRANSACTION...]\n"
           "    connect to SERVER and dump the table, row and column level\n"
           "    insert, modify and delete seqno, and the contents of the\n"
           "    database as seen initially by the IDL implementation and\n"
           "    after executing each TRANSACTION.  (Each TRANSACTION must\n"
           "    modify the database or this command will hang.)\n",
...
print_idl_seqno_info(struct ovsdb_idl *idl, int step)
{
    ...
    IDLTEST_SIMPLE_FOR_EACH (s, idl) {
        ...
        column = shash_find_data(&s->header_.table->columns, "b");
        if (column) {
            col_seqno = column->modify_seqno;
        }

        /* print i first, so that output is sorted as per expected result order */
        printf("%03d: i=%"PRId64" idlseq=%d tiseq=%d tmseq=%d tdseq=%d riseq=%d "
               "rmseq=%d cmseq=%d r=%g b=%s s=%s u="UUID_FMT" ia=[",
               step, s->i,
               ovsdb_idl_get_seqno(idl),
               s->header_.table->insert_seqno,
               s->header_.table->modify_seqno,
               s->header_.table->delete_seqno,
               s->header_.insert_seqno,
               s->header_.modify_seqno,
               col_seqno,
               s->r, s->b ? "true" : "false",
               s->s, UUID_ARGS(&s->u));
...
static void
print_idl_delete_seqno_info(struct ovsdb_idl *idl, int step)
{
    const struct idltest_simple *s;
    int n = 0;
    ...
    IDLTEST_SIMPLE_FOR_EACH (s, idl) {
        ...
        /* print i first, so that output is sorted as per expected result order */
        printf("%03d: i=%"PRId64" idlseq=%d tdseq=%d\n",
               step, s->i,
               ovsdb_idl_get_seqno(idl),
               s->header_.table->delete_seqno);
        n++;
...
/*
** Ft to test IDL table, row and column level seq number increment operations.
*/
static void
do_idl_seqno(int argc, char *argv[])
{
    struct jsonrpc *rpc;
    struct ovsdb_idl *idl;
    unsigned int seqno = 0;
    ...
    for (i = 2; i < argc; i++) {
        ...
        if (*arg == '+') {
            ...
            /* Print update. */
            if(!strcmp(argv[0], "idl-delete-seqno")) {
                print_idl_delete_seqno_info(idl, step++);
            } else {
                print_idl_seqno_info(idl, step++);
            }
    ...
    /* Print update after second set of transactions. */
    if(!strcmp(argv[0], "idl-delete-seqno")) {
        print_idl_delete_seqno_info(idl, step++);
    } else {
        print_idl_seqno_info(idl, step++);
