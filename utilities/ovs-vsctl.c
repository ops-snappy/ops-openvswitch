...
vsctl_context_populate_cache(struct vsctl_context *ctx)
{
     ...
     */
    ctx->subsystems_exist = (ovs->n_subsystems != 0);
#endif
    ...
    if (ctx->subsystems_exist) {
        ...
        OVSREC_INTERFACE_FOR_EACH (iface_cfg, ctx->idl) {
            iface = shash_find_data(&ctx->ifaces, iface_cfg->name);
            if (!iface) {
                add_orphan_iface_to_cache(ctx, iface_cfg);
            }
        }
    }
#endif
...
static void
pre_get_vlan_info(struct vsctl_context *ctx)
{
    ovsdb_idl_add_column(ctx->idl, &ovsrec_open_vswitch_col_bridges);
...
static void
cmd_list_vlans(struct vsctl_context *ctx)
{
    int i;
    struct vsctl_bridge *br;
    struct svec vlans;
...
static void
cmd_add_vlan(struct vsctl_context *ctx)
{
    int i, vid;
    char vlan_name[16];
    char *br_name;
    ...
    for (i = 0; i < n_settings; i++) {
        set_column(get_table("VLAN"), &vlan->header_, settings[i],
                   ctx->symtab);
    }
...
static void
cmd_del_vlan(struct vsctl_context *ctx)
{
    int i, vid;
    bool found = false;
    char *br_name;
    ...
    if (!found) {
        if (if_exist) {
            ...
            vsctl_fatal("Error - VLAN %d does not exist on bridge %s!\n",
                        vid, bridge->br_cfg->name);
        }
    }
...
{
    bool may_exist = shash_find(&ctx->options, "--may-exist") != NULL;
    bool fake_iface = shash_find(&ctx->options, "--fake-iface") != NULL;
    int n_ifaces;
    int i;
