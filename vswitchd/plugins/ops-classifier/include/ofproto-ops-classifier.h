...
#ifndef __VSWITCHD__OPS_CLASSIFIER__OFPROTO_OPS_CLASSIFIER_H__
#define __VSWITCHD__OPS_CLASSIFIER__OFPROTO_OPS_CLASSIFIER_H__ 1
 ...
 * PI -> PD Classifer API
 *
 * Eventually this will become an ofproto extension for ops-classifier.
 *
 * In the meanwhile, the typedefs below are added to the definition of
 * ofproto_class in: ofproto/ofproto-provider.h
 *
 ...
 * in its ofproto_class instance. Examples of this can bee seen in:
 * (opennsl plugin): src/ofproto-bcm-provider.c (container plugin):
 * src/ofproto-sim-provider.c
 *****************************************************************************/


/**************************************************************************//**
 * API from switchd platform independent layer to platform dependent layer to
 * specify the switch interface on which a given Classifier List is to be
 * applied.
 *
 * All pointer arguments reference objects whose lifetimes are not guaranteed
 * to outlast this call. If you want to remember them for later, you must
 * make a copy.
 *
 ...
 *                                should be applied
 * @param[out]  pd_status       - pointer to struct pre-allocated by calling
 *                                function
 * @retval      0               - if list successfully added to hw
 * @retval      !=0             - otherwise and update pd_status
 *
 *****************************************************************************/
typedef int (*ofproto_ops_cls_apply)(
 ...
 *
 *****************************************************************************/
typedef int (*ofproto_ops_cls_remove)(
 ...
 *
 *****************************************************************************/
typedef int (*ofproto_ops_cls_replace)(
 ...
 *****************************************************************************/
typedef int (*ofproto_ops_cls_list_update)(
    struct ops_cls_list              *list,
 ...
 *                               list priority order. array will be initialized
 *                               with 'statistics enabled' flag set to false,
 *                               entries with hitcounts enabled will be
 *                               updated with packet hitcounts from hw and will
 *                               set 'statistics enabled' flag to true.
 ...
 *                               function
 * @retval     0               - if statistics successfully retrieved from hw
 * @retval     !=0             - otherwise and updates status
 *****************************************************************************/
typedef int (*ofproto_ops_cls_statistics_get)(
    const struct uuid              *list_id,
    const char                     *list_name,
    enum ops_cls_type              list_type,
    struct ofproto                 *ofproto,
 ...
 * @retval     !=0             - otherwise and updates status
 *****************************************************************************/
typedef int (*ofproto_ops_cls_statistics_clear)(
    const struct uuid               *list_id,
    const char                      *list_name,
    enum ops_cls_type               list_type,
    struct ofproto                  *ofproto,
...
/**************************************************************************//**
 * API to clear all statistics for all applied classifier lists of all types
 * in all directions.
 *
 * All pointer arguments reference objects whose lifetimes are not guaranteed
 * to outlast this call. If you want to remember them for later, you must
 ...
 * @retval     !=0             - otherwise and updates status
 *****************************************************************************/
typedef int (*ofproto_ops_cls_statistics_clear_all)(
    struct ops_cls_pd_list_status *status);


#endif  /* __VSWITCHD__OPS_CLASSIFIER__OFPROTO_OPS_CLASSIFIER_H__ */
